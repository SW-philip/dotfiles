...
def write_swaync_css(path: Path, p: dict, name: str):
    ...
    CR = p.get("CRITICAL",        "#eb6f92")
    TC = p.get("TINT_CRITICAL_BG","#3d2030")
    path.write_text(f"""/* swaync — Papercraft — {name} — auto-generated by auto-theme.py */
* {{
...
.notification-row:focus, .notification-row:hover {{ background: none; }}
...
.close-button:hover {{ color: {LV}; background: {O}; }}
...
.time {{ font-size: 11px; font-weight: 600; color: {MU}; }}
...
.app-icon-image {{ border-radius: 6px; box-shadow: 1px 2px 0 0 {SH}; }}

.notification.critical {{ background: {TC}; border-color: {CR}; }}
.notification.critical .summary {{ color: {CR}; }}
...
.control-center-list .notification {{ margin: 4px 8px; }}
#label-count {{ font-weight: 700; color: {LV}; }}
""")
...
def activate_theme(slug: str, theme_dir: Path) -> None:
    ...
    # ── swaync ────────────────────────────────────────────────────────────────
    swaync_css_src = theme_dir / "swaync-style.css"
    swaync_css_dst = Path.home() / ".config/swaync/style.css"
    if swaync_css_src.exists():
